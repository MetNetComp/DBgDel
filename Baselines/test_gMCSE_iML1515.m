...
ori_model = iML1515;
% Read the target CSV
Targetlist = readtable('possible_target_iML1515_no_e.csv', 'Delimiter', ',');
% Extract the first column as a cell array (useful for strings)
productionList = Targetlist{:, 1}; 
% Read the iML1515_RID file (gene-pseudoreaction-ID)
map_gene_list = readtable('iML1515_RID.csv', 'Delimiter', ',');
...
maxSolutions = []; % # Solution needed for records
rkoCost = [];
rkiCost= [];
maxCost = [];
gkoCost = [];
...
writetable(results_table, 'iML1515_results.csv');
...
writetable(results_table, 'iML1515_results_check.csv'); 